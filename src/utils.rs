    ...
    async fn test_create_file_and_commit() {
        ...
        for i in 0..count {
            ...
            children.push(thread::spawn(move || {
                ...
                let file_path = utils::random_string(10);
                let file_content = utils::random_string(100);
                let commit_message = utils::random_string(100);

                let repo = utils::open_bare_repo(&repo_id.clone()).unwrap();

                match utils::create_file_then_commit_then_push(
                    &repo,
                    &file_path,
                    &file_content,
                    &branch,
